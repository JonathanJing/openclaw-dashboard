...
try:
    ...
    print("healthy 0 none 0 0 0 none 0")
    raise SystemExit(0)
...
print(status, consecutive, reason, last_alert, last_recovered, restart_attempts, last_alert_key, outage_started_at)
PY
}
...
read -r prev_status prev_failures prev_reason prev_alert_at prev_recovered_at prev_restart_attempts prev_alert_key prev_outage_started_at <<<"$(load_state)"
now="$(now_epoch)"
...
if [[ "$check_result" == "pass" ]]; then
  if [[ "$prev_status" != "healthy" ]]; then
    ...
    write_state "healthy" 0 "none" "$prev_alert_at" "$now" 0 "none" 0
  else
    write_state "healthy" 0 "none" "$prev_alert_at" "$prev_recovered_at" 0 "$prev_alert_key" 0
  fi
  exit 0
fi
...
if [[ "$should_alert" -eq 1 ]]; then
  ...
  if [[ "$AUTO_HEAL_ON_ALERT" == "1" && "$new_status" == "critical" ]]; then
    auto_heal_report="$(run_auto_heal_sequence)"
  fi
  ...
  write_state "$new_status" "$new_failures" "$reason" "$now" "$prev_recovered_at" "$restart_attempts" "$alert_key" "$outage_started_at"
else
  append_event "suppressed" "$new_status" "$reason" "告警被抑制（未到阈值或冷却中）；failures=$new_failures; details=$details"
  write_state "$new_status" "$new_failures" "$reason" "$prev_alert_at" "$prev_recovered_at" "$restart_attempts" "$prev_alert_key" "$outage_started_at"
fi
